...
MathSheety 是一个专为家长和一年级小学生设计的纯前端口算练习纸生成工具。支持自定义题型、数字范围和题数，支持显示/隐藏答案，版面完美适配 A4 纸张排版，可直接调用系统打印。
...
1. 下载或解压项目中的 `MathSheety.zip`（包含 `index.html`、`style.css`、`app.js`）。
2. 在本地计算机双击双击 `index.html` 即可直接在浏览器中运行。
