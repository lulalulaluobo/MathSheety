...
*   **2026-06-14**：新增了“元角分口算练习纸生成器”页面。创建了 `currency.html` 与 `currency.js`，支持单位换算、币值计算及等值兑换题目，并符合人民币固定面值规范。更新了 `index.html` 与 `style.css` 增加了美观的导航切换页签，针对币值长文本优化了 3 列 A4 打印网格排版。编写了 `tests/currency.test.js` 并通过了所有自动化单元测试。通过浏览器子代理验证了交互与答案显隐切换功能。
