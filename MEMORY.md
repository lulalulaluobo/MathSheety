...
*   **2026-06-17（续）**：新增**三数加减运算**题型（连加 a+b+c、连减 a-b-c、加减混合 a+b-c / a-b+c 四种形式随机）。在 `app.js` 新增 `generateThreeTerm(range, pattern)` 与 `THREE_OP_PATTERNS`，返回扩展对象 `{a,b,c,ops:['+'|'-','+'|'-'],res}`——通过 `ops` 字段与两数对象区分，保持向后兼容。难度规则：最终结果 ≤ 所选范围，且按从左到右逐步运算的中间结果均 ≥ 0（不出负数）。`renderWorksheet` 分发链追加 `three-term` 分支；去重 key 与渲染逻辑通过判断 `q.ops` 自动分流两数/三数格式。`index.html` 追加「三数加减」radio。`tests/generator.test.js` 新增测试断言中间结果非负、最终结果≤范围、四种形式都会出现。全部 8 个单元测试通过。同步更新了 `README.md`（补全乘除法/三数运算/货币页功能说明，修正离线 zip 文件清单至 5 个文件）。重新打包了 `MathSheety.zip` 分发包。
